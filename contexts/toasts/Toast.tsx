...
export const displayToast: DisplayToastFn = (
  ...
  switch (type) {
    ...
    case TToastType.WALLET_FAILED:
      return toast(
        <ToastWalletFailed message={inputExtraData.message} />,
        inputOptions
      );
    default:
...
const ToastWalletFailed: FunctionComponent<{ message: string }> = ({
  message,
}) => (
  <div className={styles.toast_content}>
    <FailedIcon />
...
const ToastTxSuccess: FunctionComponent<{
  ...
  <div className={styles.toast_content}>
    ...
    <section className={styles.toast_section}>
      ...
      {cw20Address && (
        <div
          className={styles.cw20_section}
          onClick={() => window.open(linkCw20Token)}
        >
          <span className={styles.cw20_section_label}>CW20 Token</span>:{" "}
          <span className={styles.cw20_section_value}>
            {reduceString(cw20Address, 6, 6)}
          </span>
          <LinkIcon />
        </div>
      )}
      {linkLpAddress && (
        <div
          className={styles.cw20_section}
          onClick={() => window.open(linkLpAddress)}
        >
          <span className={styles.cw20_section_label}>LP Token</span>:{" "}
          <LinkIcon />
        </div>
      )}
      {linkPairAddress && (
        <div
          className={styles.cw20_section}
          onClick={() => window.open(linkPairAddress)}
        >
          <span className={styles.cw20_section_label}>Pair contract</span>:{" "}
          <LinkIcon />
        </div>
      )}
      <a target="__blank" href={link}>
