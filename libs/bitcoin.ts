...
export default class Bitcoin {
  ...
  async getAddress(
    chainId: BitcoinChainId = bitcoinChainId
  ...
  async signAndBroadCast(
    ...
    try {
      ...
      console.error(
        "Error while signing and broadcasting Bitcoin transaction:",
        error
      );
      throw new Error(
        `Error while signing and broadcasting Bitcoin transaction:  ${JSON.stringify(
          error
        )}`
      );
    }
