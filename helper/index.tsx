...
export const getWalletByNetworkCosmosFromStorage = (
  ...
  try {
    ...
    // const wallet = JSON.parse(parsedResult.state);
    const wallet = parsedResult.state;
    // return wallet.walletsByNetwork.cosmos;
    return wallet.oraiWallet;
  } catch (error) {
    console.log("error getWalletByNetworkCosmosFromStorage: ", error);
  }
};

export const getWalletByNetworkFromStorage = (
  key = "Zus:Authentication"
): any => {
  // "persist:root"
  try {
...
//   if (
//     isMobile() ||
//     (walletType === "owallet" && window.tronLinkDapp?.isOwallet)
//   ) {
...
//           const accountTron: interfaceRequestTron =
//             await window.tronLinkDapp.request({
//               method: "tron_requestAccounts",
//             });
...
//           window.ethereumDapp = window.eth_owallet;
...
export const getListAddressCosmosByLeapSnap = async () => {
  ...
  const cosmosNetworksFilter = cosmosNetworks.filter(
    (item, index) =>
      item.chainId !== "kawaii_6886-1" && item.chainId !== "injective-1"
  );
  ...
  for (const info of cosmosNetworksFilter) {
    ...
    try {
      ...
      console.log(
        `🚀 ~ file: index.tsx:316 ~ getListAddressCosmosByLeapSnap ~ error ${info.chainId}:`,
        error
      );
    }
  }
...
export const getAddressByEIP191 = async (isSwitchWallet?: boolean) => {
  const metamaskOfflineSinger = await MetamaskOfflineSigner.connect(
    window.ethereum
  );
